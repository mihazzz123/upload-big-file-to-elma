...
import (
	...
	"git.elewise.com/elma365/common/pkg/util"
)
...
func Read(v interface{}, opts ...Option) error {
	...
	m := r.ExtractTags("envcfg", util.WithoutMinus())
	for k, v := range m {
		if v == "" {
...
func applyDefault(r util.Reflector, overrides map[string]string) error {
	m := r.ExtractTags("default", util.WithoutEmpty())
